...
static int pgfault_handler(struct trapframe *tf) {//处理页面错误异常
    extern struct mm_struct *check_mm_struct;
    print_pgfault(tf);
    if (check_mm_struct != NULL) {
        return do_pgfault(check_mm_struct, tf->cause, tf->badvaddr); //把Page Fault分发给kern/mm/vmm.c的do_pgfault()函数并尝试进行页面置换。
    }
...
void exception_handler(struct trapframe *tf) {
    ...
    switch (tf->cause) {
        ...
        case CAUSE_FETCH_PAGE_FAULT:
            ...
            break;
        case CAUSE_LOAD_PAGE_FAULT:   //取指令时发生的Page Fault先不处理

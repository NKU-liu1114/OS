...
int
swap_init(void)//初始化内存管理系统中的交换机制
{
     swapfs_init();//初始化了交换文件系统

     // Since the IDE is faked, it can only store 7 pages at most to pass the test
     //由于IDE是虚拟的，最多只能存储7页以通过测试
     if (!(7 <= max_swap_offset &&
        max_swap_offset < MAX_SWAP_OFFSET_LIMIT)) {//保 max_swap_offset 大于等于 7，且小于一个定义的上限值
...
int
swap_out(struct mm_struct *mm, int n, int in_tick)//在物理内存不足时，将内存中的页面交换出去到磁盘上
{
     int i;
     for (i = 0; i != n; ++ i)
     {
...
int
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)//将页面从磁盘换回到内存中
{
